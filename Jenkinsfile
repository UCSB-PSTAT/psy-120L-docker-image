pipeline {
    agent none
    triggers{
        upstream(upstreamProjects: 'UCSB-PSTAT GitHub/base-rstudio/int-test', threshold: hudson.model.Result.SUCCESS)
    }
    environment {
        IMAGE_NAME = 'psy120l'
    }
            ...
            stages{
                ...
                stage('Test') {
                    steps {
                        ...
                        sh 'podman run -it --rm localhost/$IMAGE_NAME R -e "library(\"psych\");library(\"afex\");library(\"Hmisc\");library(\"emmeans\")"'
